...
<br/>
<span class="title1" style="font-size:30px;"><center>View/Edit Question Details</center></span>
	<h2>Progress</h2>		
	<div class="progress">
...
<fieldset>
    ...
    <div class="form-group"> 
        <center><img id = "output" src="<?php if(@validate($qnsimg))echo $qnsimg; ?>" class = "<?php if(!@validate($qnsimg))echo "hide"; ?>"/></center><br/>
      <?php echo '<input id="clear_img" name="clear_img" type = "submit" value="Remove Image" formaction="dash_admin.php?q=17&step=3&eid='.$exam_id.'&n='.@$_GET['n'].'"   class="form-control" >'; ?>
    </div>
	...
	<select id="ans<?php echo $i?>" name="ans<?php echo $i?>" placeholder="Choose correct answer " class="form-control input-md" >
      ...
      <option value="d" <?php if($id_d == $answer_id){
        ...
        <?php
if(@validate($_POST['clear_img'])){
	if(file_exists($_SESSION["qnsimg"])){
		unlink ($_SESSION["qnsimg"]);	
	}
	mysqli_query($con,"UPDATE obj_questions SET question_imagepath = ''  WHERE question_id = '$question_id'") or die('Error');
}
