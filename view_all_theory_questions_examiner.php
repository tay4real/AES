...
<input name="name" placeholder="Search Examiner/Subjects" autocomplete="off" type="text" id="name" size="150" onkeyup="doSearch()">
  <div id="result">
  	<?php
		
		echo  '<div class="panel"><div class="table-responsive" style="overflow:scroll;max-height:430px ">
		<span class="title1" style="margin-left:40%;font-size:30px;"><center><img src="image/view_theory_qtn.png" /></center></span><br /><br />
		...
		$get_EXAMINER_id=get_Theory_EXAMINER_id($examiner_id);
		...
		while($row=$get_EXAMINER_id->fetch_array() )
		{
			...
			echo '<tr><td>'.$c++.'</td><td>'.$school_code.'</td><td><a href="dash_examiner.php?q=23&step=3&eid='.$exam_id.'&n='.$no_of_questions.'&sid='.$subject_id.'">'.$exam_code.'</a></td><td>'.$exam_name.'</td><td>'.$class.'</td><td>'.$no_of_questions.'</td><td>'.$fullname.'</td><td>'.$date_performed.'</td>	
			
			<td> <a title="View/Update Theory Question" href="dash_examiner.php?q=23&step=3&eid='.$exam_id.'&n='.$no_of_questions.'&sid='.$subject_id.'"><b><span class="glyphicon glyphicon-edit" aria-hidden="true"></span></b></a> </td>
