  	...
  	<?php
		...
		while($row5=$getTheoryExam_code->fetch_array() )
		{
			...
			while($row=$getExaminedStudentRegNo->fetch_array() )
			{
				...
				echo '<tr><td>'.$c++.'</td><td>'.$name.'</td><td><a title="View answer sheet for this student" href="dash_admin.php?q=30&e='.$exam_id.'&examiner_id='.$examiner_id.'&regno='.$regno.'&name='.$name.'&class_id='.$class.'&subject_id='.$subject_id.'">'.$regno.'</a></td><td>'.$school_name.'</td><td>'.$subject_id.'</td><td>'.$class.'</td><td>'.$status.'</td>
				
				<td> <a title="Delete This Record" onclick="return verifyDelete()" href="update.php?del_obj_question='.$exam_id.'"><b><span class="glyphicon glyphicon-trash" aria-hidden="true"></span></b></a> </td>
