  ...
  <div id="result">
  	<?php
		...
		while($row=$allTheory_Questions->fetch_array() ) {
			...
			echo '<tr><td>'.$c++.'</td><td>'.$school_code.'</td><td><a href="dash_admin.php?q=26&step=3&eid='.$exam_id.'&n='.$no_of_questions.'">'.$exam_code.'</a></td><td>'.$exam_name.'</td><td>'.$class.'</td><td>'.$no_of_questions.'</td><td>'.$examiner_id.'</td><td>'.$date_performed.'</td>			
