...
<input name="name" placeholder="Search Examiner/Subjects" type="text" id="name" size="150" onkeyup="doSearch()">
  <div id="result">
  	<?php
		...
		<span class="title1" style="font-size:30px;"><center><img src="image/theory_result.png" /></center></span><br /><br />
		...
		while($row=$allTheory_Questions->fetch_array() ) {
			...
			$res = mysqli_query($con, "SELECT DISTINCT regno FROM `theory_exam_submitted` WHERE exam_id = '$exam_id'");
			$count = mysqli_num_rows($res);
			
					
			//$total_students
			
			echo '<tr><td>'.$c++.'</td><td>'.$school_code.'</td><td><a href="dash_admin.php?q=26&e='.$exam_id.'&examiner_id='.$examiner_id.'">'.$exam_code.'</a></td><td>'.$exam_name.'</td><td>'.$class.'</td><td>'.$no_of_questions.'</td><td>'.$count.'</td>
			
			
			
			<td> <center><a title="Check Objective Exam Results" href="dash_admin.php?q=26&e='.$exam_id.'&examiner_id='.$examiner_id.'"><b><span class="glyphicon glyphicon-file" aria-hidden="true"></span></b></a></center> </td>
