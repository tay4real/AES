...
<div class="row" style="background-color:#fff; padding-top:30px; padding-bottom:30px">
 ...
 <form class="form-horizontal title1" name="form" action="update_exami_admin.php"  method="POST" enctype="multipart/form-data">
...
<form class="form-horizontal title1" name="form" action="update_subject_admin.php"  method="POST" enctype="multipart/form-data">
<hr>
<div class="w3-row w3-padding">
...
<div class="col-md-8" >
<select id="sub" name="more_sub"  class="form-control input-md">
    <option value="">Select Subject</option>
    <?php 
    // Fetch Department
    $sql_subject = "SELECT * FROM subject_ids";
    $subject_data = mysqli_query($con,$sql_subject);
    while($row = mysqli_fetch_assoc($subject_data) ){
        $subject_id = $row['id'];
        $subject = $row['subject'];
